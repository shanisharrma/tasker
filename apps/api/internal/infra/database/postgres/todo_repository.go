...
func (r *TodoRepository) Create(ctx context.Context, userID string, payload *todo.CreateTodoPayload) (*todo.Todo, error) {
	stmt := `
		INSERT INTO
			todos (
				user_id,
...
func (r *TodoRepository) GetByID(ctx context.Context, userID string, todoID uuid.UUID) (*todo.PopulatedTodo, error) {
	stmt := `
		SELECT 
			t.*,
			CASE
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute get todo by id query for todo_id=%s user_id=%s: %w", todoID, userID, err)
	}

	todoItem, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[todo.PopulatedTodo])
	if err != nil {
		return nil, fmt.Errorf("failed to collect row from table:todos for todo_id=%s user=%s: %w", todoID, userID, err)
	}

	return &todoItem, nil
}

func (r *TodoRepository) CheckExists(ctx context.Context, userID string, todoID uuid.UUID) (*todo.Todo, error) {
	stmt := `
		SELECT 
			*
		FROM
	...
	if err != nil {
		return nil, fmt.Errorf("failed to check if todo exists for todo_id=%s user_id=%s: %w", todoID, userID, err)
	}
...
func (r *TodoRepository) List(ctx context.Context, userID string, query *todo.GetTodosQuery) (*types.PaginatedResponse[todo.PopulatedTodo], error) {
	stmt := `
		SELECT 
			t.*,
			CASE
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return &types.PaginatedResponse[todo.PopulatedTodo]{
...
func (r *TodoRepository) Update(ctx context.Context, userID string, payload *todo.UpdateTodoPayload) (*todo.Todo, error) {
...
func (r *TodoRepository) Delete(ctx context.Context, userID string, todoID uuid.UUID) error {
	stmt := `
		DELETE FROM todos
		WHERE
			id=@todo_id
...
func (r *TodoRepository) GetStats(ctx context.Context, userID string) (*todo.TodoStats, error) {
	stmt := `
		SELECT 
			COUNT(*) AS total,
			COUNT(
