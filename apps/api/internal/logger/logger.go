...
// NewLoggerService create a new logger service with New Relic integration
...
// NewLogger create a new logger with specified level (backward compatibility)
...
// NewLoggerWithConfig create a logger with full config (backward compatibility)
func NewLoggerWithConfig(cfg *config.ObservabilityConfig) zerolog.Logger {
	return NewLoggerWithService(cfg, nil)
}

// NewLoggerWithService create a logger with full config and logger service
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	logger := zerolog.New(writer).
		...
		Str("enviroment", cfg.Environment).
		Logger()
...
// GetPgsTraceLogLevel converts zerolog level to pgx tracelog level
func GetPgxTraceLogLevel(level zerolog.Level) int {
	switch level {
	case zerolog.DebugLevel:
		return 6 // tracelog.LogLevelDebug
	case zerolog.InfoLevel:
		return 4 // tracelog.LogeLevelInfo
